...
%geometries = mc_300_geo; %change name each run
...
%materials = mc_300_inputs'; %change name each run
...
if mat_geo_coupled
    ...
    if n_geometries ~= n_materials
        ...
        rho_soft = materials(:,1);
        rho_hard = materials(:,2);
        K_soft = materials(:,3);
        K_hard = materials(:,4);
        G_soft = materials(:,5);
        G_hard = materials(:,6);

        E_soft = (9*K_soft.*G_soft)./(3*K_soft+G_soft);
        E_hard = (9*K_hard.*G_hard)./(3*K_hard+G_hard);
        pr_soft = (3*K_soft-2*G_soft)./(6*K_soft+2*G_soft);
        pr_hard = (3*K_hard-2*G_hard)./(6*K_hard+2*G_hard);      

        tic;
        parfor i = 1:n_geometries
    ...
    if save_outputs
        ...
        save(['K_soft_gaussian_' num2str(materials_size(2)) 'd_fp_5%_n' num2str(length(K_soft))], 'K_soft');
        save(['K_hard_gaussian_' num2str(materials_size(2)) 'd_fp_5%_n' num2str(length(K_hard))], 'K_hard');
        save(['G_soft_gaussian_' num2str(materials_size(2)) 'd_fp_5%_n' num2str(length(K_soft))], 'G_soft');
        save(['G_hard_gaussian_' num2str(materials_size(2)) 'd_fp_5%_n' num2str(length(K_hard))], 'G_hard');
...
if ~mat_geo_coupled
    ...
    elapsedTime = 0;
    tic;
    parfor g = 1:n_geometries
        parfor i = 1:n_materials
            [EIGENVALUE_DATA, WAVEVECTOR_DATA] = dispersion_data_custom_mat_geo_param( ...
                E_soft(i), ...
                E_hard(i), ...
                rho_soft(i), ...
