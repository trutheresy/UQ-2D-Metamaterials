function [wavevectors,contour_info] = get_IBZ_contour_wavevectors(N_k,a,symmetry_type)
    ...
    function linspaceNDimPoints = linspaceNDim(vertices_start, vertices_end, N_d)
        %disp(length(vertices_start))
        %disp(vertices_end)
        linspaceNDimPoints = zeros(length(vertices_start), N_d);
        for i = 1:length(vertices_start)
            %disp(size(linspaceNDimPoints))
            %disp(vertices_start(i))
            %disp(vertices_end(i))
            linspaceNDimPoints(i,:) = linspace(vertices_start(i), vertices_end(i),N_d);
        end
    end    
end
