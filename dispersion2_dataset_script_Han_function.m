function [bg_size, bg_bottom, bg_top] = Han_bandgap(E_max, E_min)
    hanProject = true;
    isSaveOutput = false;
    ...
    % for struct_idx = 1:N_struct
        pfc = const;
        if hanProject
            design(:,:,1) = ones(const.N_pix); % the first pane is E
        ...
        elseif struct_idx == 1 && isIncludeHomogeneous
            pfc.design = get_design('dispersive-tetragonal',pfc.N_pix);
            pfc.design = convert_design(pfc.design,'linear',pfc.design_scale,pfc.E_min,pfc.E_max,pfc.rho_min,pfc.rho_max);
        else
